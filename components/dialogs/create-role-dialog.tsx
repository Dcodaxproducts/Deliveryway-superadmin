...
export function CreateRoleDialog() {
  ...
  useEffect(() => {
    setSelectedPermissions((prev) => {
      const sanitized = sanitizePermissions(
        Object.entries(prev).map(([access, operations]) => ({
          access,
          operations,
        })),
        permissionModules,
      );
      const nextPermissions = sanitized.reduce<Record<string, string[]>>(
        (next, permission) => {
      ...
      return JSON.stringify(prev) === JSON.stringify(nextPermissions)
        ? prev
        : nextPermissions;
    });
  }, [permissionModules]);
  ...
  const handleSubmit = () => {
    ...
    const permissions = sanitizePermissions(
      Object.entries(selectedPermissions).map(([access, operations]) => ({
        access,
        operations,
      })),
      permissionModules,
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="primary">{rbac("addNewRole")}</Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[618px] bg-[#F5F5F5] p-[40px] border-none shadow-lg rounded-[14px] max-h-[90vh] overflow-y-auto">
        <DialogHeader className="text-left">
          ...
          <DialogDescription>{rbac("createRoleDescription")}</DialogDescription>
        </DialogHeader>
        ...
        <div className="grid gap-6 mt-[32px] p-[24px] bg-white rounded-[14px] shadow-sm">
          ...
          <div>
            <h3 className="text-base text-dark mb-[6px]">
              {rbac("permissions")}
            </h3>
                      ...
                      {module.defaultActions.map((operation) => (
                        ...
                        >
                          <Checkbox
                            ...
                            checked={
                              selectedPermissions[module.accessKey]?.includes(
                                operation,
                              ) || false
                            }
            ...
            {permissionError ? (
              <p className="mt-3 text-sm text-primary">
                Select at least one permission action.
              </p>
            ) : null}
          </div>
        </div>
